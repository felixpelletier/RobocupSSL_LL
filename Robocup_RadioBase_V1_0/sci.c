...
//added by Mathieu Garon:

bool SCI_RxOverflow(SCI_Handle sciHandle){
	SCI_Obj *sci = (SCI_Obj *)sciHandle;

	if(sci->SCIFFRX & SCI_SCIFFRX_FIFO_OVF_BITS){
		return true;
	}
	else{
		return false;
...
uint16_t SCI_get_SCIFFRX(SCI_Handle sciHandle){
	SCI_Obj *sci = (SCI_Obj *)sciHandle;
	return sci->SCIFFRX;
}

uint16_t SCI_get_SCIBAUD(SCI_Handle sciHandle){
	SCI_Obj *sci = (SCI_Obj *)sciHandle;
	uint16_t baud = 0;
	baud = sci->SCIHBAUD << 8;
	baud |= sci->SCILBAUD;
