...
quad_Handle quad_init(chip_select pCSNPin){
    ...
    quad_WriteRegister(QUAD_CONFIG0, 0x0700, &lHandle);  //counter0 = 16 bits and counter1 = 16 bit (no counter2)
    quad_WriteRegister(QUAD_CONFIG1, 0x8000, &lHandle);  // counter TTL
    return lHandle;
}
...
uint16_t quad_readRegister(uint16_t pReg,quad_Handle *pQuad){
	...
	SPI_write(HandleRobot.HandleSPI, pReg | 0x8000); //put a 0 on the first
	SPI_write(HandleRobot.HandleSPI, pReg); //dummy data
...
void quad_WriteRegister(uint16_t pReg, uint16_t pValue,quad_Handle *pQuad){
	demux_connect_to(pQuad->structCSNPin);
	SPI_write(HandleRobot.HandleSPI, pReg & ~0x8000); //put a 0 on the first
	SPI_write(HandleRobot.HandleSPI, pValue); //dummy data
	while(SPI_getRxFifoStatus(HandleRobot.HandleSPI) < SPI_FifoStatus_2_Words); //wait for two words (STATUS + REG)
...
void quad_readCounters(quad_Handle *pQuad){
	...
	demux_connect_to(pQuad->structCSNPin);
	SPI_write(HandleRobot.HandleSPI, QUAD_COUNTER | 0x8000); //put a 0 on the first
